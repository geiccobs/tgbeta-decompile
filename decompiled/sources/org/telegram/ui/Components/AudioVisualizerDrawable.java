...
public class AudioVisualizerDrawable {
    ...
    public AudioVisualizerDrawable() {
        for (int i = 0; i < 2; i++) {
            ...
            CircleBezierDrawable drawable = new CircleBezierDrawable(6);
            circleBezierDrawableArr[i] = drawable;
            drawable.idleStateDiff = 0.0f;
            drawable.radius = AndroidUtilities.dp(24.0f);
    ...
    public void setWaveform(boolean playing, boolean animate, float[] waveform) {
        ...
        if (playing || animate) {
            ...
            if (waveform != null && amplitude > 0.4d) {
            ...
            if (idleState) {
                for (int i3 = 0; i3 < 6; i3++) {
                    waveform[i3] = (this.random.nextInt() % 500) / 1000.0f;
                }
            }
    ...
    public void draw(Canvas canvas, float cx, float cy) {
        ...
        float radius = AndroidUtilities.dp(22.0f) + (AndroidUtilities.dp(4.0f) * radiusProgress) + (this.IDLE_RADIUS * enterProgress);
        if (radius > AndroidUtilities.dp(26.0f)) {
            radius = AndroidUtilities.dp(26.0f);
        }
        CircleBezierDrawable[] circleBezierDrawableArr = this.drawables;
