    ...
    public boolean create() {
        ...
        while (true) {
            ...
            if (i5 >= (z2 ? 2 : 1)) {
                ...
                GLES20.glAttachShader(this.enhanceShaderProgram, loadShader10);
                ...
                GLES20.glAttachShader(this.greenAndBlueChannelOverlayProgram, loadShader11);
                ...
                GLES20.glAttachShader(this.highPassProgram, loadShader12);
                ...
                GLES20.glAttachShader(this.boostProgram, loadShader14);
                ...
                GLES20.glAttachShader(this.compositeProgram, loadShader16);
            ...
            if (i5 == 1 && z2) {
                int loadShader17 = loadShader(35633, "attribute vec4 position;uniform mat4 videoMatrix;attribute vec4 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = vec2(videoMatrix * inputTexCoord).xy;}");
                Locale locale2 = Locale.US;
                str2 = str5;
                Object[] objArr = new Object[2];
                objArr[c] = str3;
                objArr[1] = str4;
                i4 = loadShader(35632, String.format(locale2, "%1$s\nprecision highp float;varying vec2 texCoord;uniform %2$s sourceImage;vec3 rgb_to_hsv(vec3 c) {vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);vec4 p = c.g < c.b ? vec4(c.bg, K.wz) : vec4(c.gb, K.xy);vec4 q = c.r < p.x ? vec4(p.xyw, c.r) : vec4(c.r, p.yzx);float d = q.x - min(q.w, q.y);float e = 1.0e-10;return vec3(abs(q.z + (q.w - q.y) / (6.0 * d + e)), d / (q.x + e), q.x);}void main() {vec4 texel = texture2D(sourceImage, texCoord);gl_FragColor = vec4(rgb_to_hsv(texel.rgb), texel.a);}", objArr));
                i3 = loadShader17;
            } else {
                str2 = str5;
                int loadShader18 = loadShader(35633, "attribute vec4 position;attribute vec2 inputTexCoord;varying vec2 texCoord;void main() {gl_Position = position;texCoord = inputTexCoord;}");
            ...
            if (i4 > photoSize || this.renderBufferHeight > photoSize || i % 360 != 0) {
                ...
                if (i4 > photoSize || this.renderBufferHeight > photoSize) {
                    ...
                    if (f < height) {
                        ...
                        this.renderBufferHeight = (int) (bitmap.getHeight() * f);
                    } else {
                ...
                bitmap2 = createBitmap(bitmap2, i, f);
            }
            GLES20.glBindTexture(3553, this.bitmapTextre[0]);
            GLES20.glTexParameteri(3553, 10241, 9729);
            GLES20.glTexParameteri(3553, 10240, 9729);
