...
public class DoubleLinearChartView extends BaseChartView<DoubleLinearChartData, LineViewData> {
    ...
    protected void drawChart(Canvas canvas) {
        ...
        if (this.chartData != 0) {
            ...
            if (this.transitionMode == 2) {
                ...
                canvas.save();
                canvas.scale(this.transitionParams.progress, this.transitionParams.progress, this.transitionParams.pX, this.transitionParams.pY);
                ...
                if (line.enabled || line.alpha != f) {
                    ...
                    while (i3 <= localEnd2) {
                        if (y[i3] < 0) {
                            ...
                            offset2 = offset3;
                            localStart = localStart2;
                            float yPercentage = ((y[i3] * ((DoubleLinearChartData) this.chartData).linesK[k]) - this.currentMinHeight) / (this.currentMaxHeight - this.currentMinHeight);
                            float padding = line.paint.getStrokeWidth() / 2.0f;
                            int measuredHeight = getMeasuredHeight();
                            localEnd = localEnd2;
                            int localEnd3 = this.chartBottom;
                            float yPoint = ((getMeasuredHeight() - this.chartBottom) - padding) - ((((measuredHeight - localEnd3) - SIGNATURE_TEXT_HEIGHT) - padding) * yPercentage);
                            if (USE_LINES) {
                                if (j == 0) {
                                    int j2 = j + 1;
                ...
                if (line.enabled || line.alpha != 0.0f) {
                    ...
                    for (int i = 0; i < n; i++) {
                        if (y[i] >= 0) {
                            ...
                            float h = ANIMATE_PICKER_SIZES ? this.pickerMaxHeight : ((DoubleLinearChartData) this.chartData).maxValue;
                            float yPercentage = (y[i] * ((DoubleLinearChartData) this.chartData).linesK[k]) / h;
                            float yPoint = (1.0f - yPercentage) * (bottom - top);
                            if (USE_LINES) {
                                if (j == 0) {
                                    int j2 = j + 1;
    ...
    public void drawSelection(Canvas canvas) {
        ...
        float xPoint = (((DoubleLinearChartData) this.chartData).xPercentage[this.selectedIndex] * fullWidth) - offset;
        ...
        while (true) {
            ...
            if (this.tmpI < this.tmpN) {
                ...
                if (line.enabled || line.alpha != 0.0f) {
                    float yPercentage = ((line.line.y[this.selectedIndex] * ((DoubleLinearChartData) this.chartData).linesK[this.tmpI]) - this.currentMinHeight) / (this.currentMaxHeight - this.currentMinHeight);
    ...
    public void drawSignaturesToHorizontalLines(Canvas canvas, ChartHorizontalLinesData a) {
        ...
        this.linePaint.setAlpha((int) (a.alpha * 0.1f * transitionAlpha));
        ...
        for (int i = 0; i < n; i++) {
            int y = (int) ((getMeasuredHeight() - this.chartBottom) - (chartHeight * ((a.values[i] - this.currentMinHeight) / (this.currentMaxHeight - this.currentMinHeight))));
            if (a.valuesStr != null && this.lines.size() > 0) {
                if (a.valuesStr2 != null && this.lines.size() >= 2) {
                    this.signaturePaint.setColor(((LineViewData) this.lines.get(leftIndex)).lineColor);
                    this.signaturePaint.setAlpha((int) (a.alpha * ((LineViewData) this.lines.get(leftIndex)).alpha * transitionAlpha * additionalOutAlpha));
                    canvas.drawText(a.valuesStr[i], HORIZONTAL_PADDING, y - textOffset, this.signaturePaint);
                }
            ...
            if (a.valuesStr2 != null && this.lines.size() > 1) {
                this.signaturePaint2.setColor(((LineViewData) this.lines.get(rightIndex)).lineColor);
                this.signaturePaint2.setAlpha((int) (a.alpha * ((LineViewData) this.lines.get(rightIndex)).alpha * transitionAlpha * additionalOutAlpha));
                canvas.drawText(a.valuesStr2[i], getMeasuredWidth() - HORIZONTAL_PADDING, y - textOffset, this.signaturePaint2);
            }
        }
    }
    ...
    public int findMaxValue(int startXIndex, int endXIndex) {
        ...
        int max = 0;
        for (int i = 0; i < n; i++) {
            int localMax = ((LineViewData) this.lines.get(i)).enabled ? (int) (((DoubleLinearChartData) this.chartData).lines.get(i).segmentTree.rMaxQ(startXIndex, endXIndex) * ((DoubleLinearChartData) this.chartData).linesK[i]) : 0;
            if (localMax > max) {
                max = localMax;
            }
        }
    ...
    public int findMinValue(int startXIndex, int endXIndex) {
        ...
        for (int i = 0; i < n; i++) {
            int localMin = ((LineViewData) this.lines.get(i)).enabled ? (int) (((DoubleLinearChartData) this.chartData).lines.get(i).segmentTree.rMinQ(startXIndex, endXIndex) * ((DoubleLinearChartData) this.chartData).linesK[i]) : Integer.MAX_VALUE;
            if (localMin < min) {
                min = localMin;
            }
        }
    ...
    public void updatePickerMinMaxHeight() {
        ...
        if (this.lines.size() > 1) {
            max = (int) (max * ((DoubleLinearChartData) this.chartData).linesK[1]);
        }
        if (max > 0 && max != this.animatedToPickerMaxHeight) {
            ...
            this.pickerAnimator = createAnimator(this.pickerMaxHeight, this.animatedToPickerMaxHeight, new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.Charts.DoubleLinearChartView.1
