...
public abstract class FourierTransform {
    ...
    protected void fillSpectrum() {
        ...
        if (i2 == 1) {
            ...
            while (true) {
                ...
                if (i4 < i5) {
                    if (i4 != 0) {
                        lowFreq = (this.sampleRate / 2) / ((float) Math.pow(2.0d, i5 - i4));
                    } else {
                        lowFreq = 0.0f;
                    }
                    float hiFreq = (this.sampleRate / 2) / ((float) Math.pow(2.0d, (this.octaves - i4) - 1));
                    float freqStep = (hiFreq - lowFreq) / this.avgPerOctave;
                    float f2 = lowFreq;
                    int j2 = 0;
                    while (true) {
    ...
    public int freqToIndex(float freq) {
        ...
        float fraction = freq / this.sampleRate;
        return Math.round(this.timeSize * fraction);
    }
    ...
    public float indexToFreq(int i) {
        ...
        if (i == this.spectrum.length - 1) {
            float lastBinBeginFreq = (this.sampleRate / 2) - (bw / 2.0f);
            float binHalfWidth = 0.25f * bw;
            return lastBinBeginFreq + binHalfWidth;
        }
        float binHalfWidth2 = i;
        return binHalfWidth2 * bw;
    }
