package j$.util.stream;

import java.util.Arrays;
/* loaded from: classes2.dex */
final class K3 extends J0 {
    public K3(AbstractC0066c abstractC0066c) {
        super(abstractC0066c, EnumC0083e4.INT_VALUE, EnumC0077d4.q | EnumC0077d4.o);
    }

    @Override // j$.util.stream.AbstractC0066c
    public A1 E0(AbstractC0193y2 abstractC0193y2, j$.util.u uVar, j$.util.function.m mVar) {
        if (EnumC0077d4.SORTED.d(abstractC0193y2.s0())) {
            return abstractC0193y2.p0(uVar, false, mVar);
        }
        int[] iArr = (int[]) ((AbstractC0182w1) abstractC0193y2.p0(uVar, true, mVar)).e();
        Arrays.sort(iArr);
        return new C0069c2(iArr);
    }

    @Override // j$.util.stream.AbstractC0066c
    public AbstractC0130m3 H0(int i, AbstractC0130m3 abstractC0130m3) {
        abstractC0130m3.getClass();
        return EnumC0077d4.SORTED.d(i) ? abstractC0130m3 : EnumC0077d4.SIZED.d(i) ? new P3(abstractC0130m3) : new H3(abstractC0130m3);
    }
}
