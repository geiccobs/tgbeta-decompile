    ...
    void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                scaledTime = (MATERIAL_INTERPOLATOR.getInterpolation(scaledTime2) * 0.79f) + 0.01f + startTrim;
            } else {
                float scaledTime3 = (interpolatedTime - 0.5f) / 0.5f;
                float endTrim = ring.getStartingStartTrim() + 0.79f;
                scaledTime = endTrim;
                startTrim = endTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation(scaledTime3)) * 0.79f) + 0.01f);
            }
