        ...
        while (layoutState.hasMore(state) && (this.mLayoutState.mInfinite || !this.mRemainingSpans.isEmpty())) {
            ...
            if (assignSpan) {
                if (lp.mFullSpan) {
                    ...
                    char c = z ? 1 : 0;
                    char c2 = z ? 1 : 0;
                    currentSpan3 = spanArr[c];
                } else {
                    currentSpan3 = getNextSpan(layoutState);
                }
            ...
            if (layoutState.mLayoutDirection == r10) {
                ...
                if (lp.mFullSpan) {
                    i2 = getMinStart(defaultNewViewLine);
                } else {
                    i2 = currentSpan.getStartLine(defaultNewViewLine);
                }
                end = i2;
            ...
            if (lp.mFullSpan && layoutState.mItemDirection == -1) {
                if (assignSpan) {
                    ...
                    if (layoutState.mLayoutDirection == r10) {
                        hasInvalidGap = (areAllEndsEqual() ? 1 : 0) ^ r10;
                    } else {
                        boolean hasInvalidGap2 = areAllStartsEqual();
                        boolean z2 = r10 == true ? 1 : 0;
                        boolean z3 = r10 == true ? 1 : 0;
                        hasInvalidGap = (hasInvalidGap2 ? 1 : 0) ^ z2;
                    }
                    if (hasInvalidGap) {
            ...
            if (isLayoutRTL() && this.mOrientation == r10) {
                if (lp.mFullSpan) {
                    ...
                    int endAfterPadding = this.mSecondaryOrientation.getEndAfterPadding();
                    int i5 = this.mSpanCount;
                    int i6 = r10 == true ? 1 : 0;
                    int i7 = r10 == true ? 1 : 0;
                    otherEnd2 = endAfterPadding - (((i5 - i6) - currentSpan.mIndex) * this.mSizePerSpan);
                }
                otherEnd = otherEnd2;
                otherStart = otherEnd2 - this.mSecondaryOrientation.getDecoratedMeasurement(view);
            } else {
                int otherStart2 = lp.mFullSpan ? this.mSecondaryOrientation.getStartAfterPadding() : (currentSpan.mIndex * this.mSizePerSpan) + this.mSecondaryOrientation.getStartAfterPadding();
