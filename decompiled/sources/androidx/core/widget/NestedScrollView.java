...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode != 0 && getChildCount() > 0) {
            ...
            if (childSize < parentSpace) {
                ...
                int childHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(parentSpace, C.BUFFER_FLAG_ENCRYPTED);
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (!canScroll) {
            ...
            return (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(TsExtractor.TS_STREAM_TYPE_HDMV_DTS)) ? false : true;
            ...
            switch (event.getKeyCode()) {
                ...
                case 20:
                    if (!event.isAltPressed()) {
                        boolean handled3 = arrowScroll(TsExtractor.TS_STREAM_TYPE_HDMV_DTS);
                        return handled3;
                    }
                    boolean handled4 = fullScroll(TsExtractor.TS_STREAM_TYPE_HDMV_DTS);
                    return handled4;
                case 62:
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (!this.mIsBeingDragged) {
                        ...
                        if (!canOverscroll) {
                            ...
                            if (pulledToY < 0) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowTop, deltaY5 / getHeight(), ev.getX(activePointerIndex) / getWidth());
                                if (!this.mEdgeGlowBottom.isFinished()) {
                                    this.mEdgeGlowBottom.onRelease();
                                }
                            } else if (pulledToY > range) {
                                EdgeEffectCompat.onPull(this.mEdgeGlowBottom, deltaY5 / getHeight(), 1.0f - (ev.getX(activePointerIndex) / getWidth()));
