...
public class FontRequestWorker {
    ...
    public static Typeface requestFontSync(final Context context, final FontRequest request, final CallbackWithHandler callback, final int style, int timeoutInMillis) {
    ...
    public static Typeface requestFontAsync(final Context context, final FontRequest request, final int style, final Executor executor, final CallbackWithHandler callback) {
        final String id = createCacheId(request, style);
        Typeface cached = sTypefaceCache.get(id);
        if (cached != null) {
            callback.onTypefaceResult(new TypefaceResult(cached));
        ...
        synchronized (LOCK) {
            ...
            Callable<TypefaceResult> fetcher = new Callable<TypefaceResult>() { // from class: androidx.core.provider.FontRequestWorker.3
                @Override // java.util.concurrent.Callable
                public TypefaceResult call() {
                    TypefaceResult typeface = FontRequestWorker.getFontSync(id, context, request, style);
                    return typeface;
                }
            };
