...
final class TtmlNode {
    ...
    public static TtmlNode buildTextNode(String text) {
        return new TtmlNode(null, TtmlRenderUtil.applyTextElementSpacePolicy(text), C.TIME_UNSET, C.TIME_UNSET, null, null, "", null);
    }
    ...
    public List<Cue> getCues(long timeUs, Map<String, TtmlStyle> globalStyles, Map<String, TtmlRegion> regionMap, Map<String, String> imageMap) {
        ...
        Iterator<Pair<String, String>> it = regionImageOutputs.iterator();
        while (it.hasNext()) {
            Pair<String, String> regionImagePair = it.next();
            ...
            if (encodedBitmapData != null) {
                ...
                cues.add(new Cue(bitmap, region.position, 0, region.line, region.lineAnchor, region.width, region.height));
                it = it;
            }
        }
        for (Map.Entry<String, SpannableStringBuilder> entry : regionTextOutputs.entrySet()) {
            TtmlRegion region2 = regionMap.get(entry.getKey());
            cues.add(new Cue(cleanUpText(entry.getValue()), (Layout.Alignment) null, region2.line, region2.lineType, region2.lineAnchor, region2.position, Integer.MIN_VALUE, region2.width, region2.textSizeType, region2.textSize));
        }
        return cues;
    }
