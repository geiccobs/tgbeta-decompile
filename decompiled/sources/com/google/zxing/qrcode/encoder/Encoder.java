...
public final class Encoder {
    ...
    public static QRCode encode(String content, ErrorCorrectionLevel ecLevel, Map<EncodeHintType, ?> hints) throws WriterException {
        ...
        if (mode == Mode.BYTE && hasEncodingHint && (eci = CharacterSetECI.getCharacterSetECIByName(encoding)) != null) {
            appendECI(eci, headerBits);
        }
        if (hints == null || !hints.containsKey(EncodeHintType.GS1_FORMAT)) {
        ...
        if (hints != null && hints.containsKey(EncodeHintType.QR_MASK_PATTERN)) {
            int hintMaskPattern2 = Integer.parseInt(hints.get(EncodeHintType.QR_MASK_PATTERN).toString());
            int maskPattern = QRCode.isValidMaskPattern(hintMaskPattern2) ? hintMaskPattern2 : -1;
            hintMaskPattern = maskPattern;
        } else {
            hintMaskPattern = -1;
        }
        if (hintMaskPattern == -1) {
            hintMaskPattern = chooseMaskPattern(finalBits, ecLevel, version, matrix);
        }
        qrCode.setMaskPattern(hintMaskPattern);
        MatrixUtil.buildMatrix(finalBits, ecLevel, version, hintMaskPattern, matrix);
    ...
    static void appendKanjiBytes(String content, BitArray bits) throws WriterException {
        try {
            ...
            for (int i = 0; i < maxI; i += 2) {
                ...
                int encoded = ((subtracted >> 8) * PsExtractor.AUDIO_STREAM) + (subtracted & 255);
